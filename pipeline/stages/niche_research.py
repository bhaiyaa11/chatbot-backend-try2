...
async def _extract_project_intelligence(metadata: dict) -> tuple:
    """
    Phase 1: Search the entire web for real facts about this project.
    1. Extract company + project from prompt
    2. Tavily search — top 5 results per query, full content included
    3. Gemini summarises everything into structured facts

    Works for ANY client, ANY project — zero setup required.
    Returns (project_intelligence_text, entities_dict)
    """
...
class NicheResearchStage:
    ...
    async def run(self, metadata: dict) -> StageResult:
        try:
            # ── Phase 1: Project Intelligence ────────────────────
            logger.info("[NicheResearch] Phase 1 — Extracting project intelligence...")
            project_intel, entities = await _extract_project_intelligence(metadata)
...
# YOUTUBE QUERIES — STRICT RULES FOR MAXIMUM TRANSCRIPT AVAILABILITY:
# You MUST only generate queries that target these specific content types which
# have 90%+ caption availability:
# - TED or TEDx talks (always captioned): "TEDx [buyer_persona] [topic]"
# - McKinsey, Deloitte, Gartner, Forrester channel content (always captioned)
# - University lecture or business school keynote on the topic
# - Major conference keynotes: Davos, Web Summit, Salesforce Dreamforce, etc.
# - Always include the word "keynote" or "talk" or "lecture" in the query

# NEVER generate queries targeting:
# - Brand videos or product launches (rarely captioned)
# - Company channel content (inconsistent captions)
# - Interview shows or podcasts on YouTube (often no captions)

# Good: "TEDx Chief Sustainability Officer climate data talk"
# Good: "McKinsey aerospace digital transformation keynote"
# Good: "Davos panel ESG reporting challenges 2024"
# Bad: "Capgemini sustainability video 2024"
# Bad: "IBM product launch keynote"
...
# BUYER LANGUAGE (real unpolished language from Reddit and reviews — use these exact phrases):
# {buyer_language}
...
# # ==================================================
# # Buyer language — Reddit + G2 via Tavily
# # ==================================================

# async def _fetch_buyer_language(
#     buyer_persona: str,
#     industry: str,
#     company: str,
# ) -> str:
#     queries = [
#         f"site:reddit.com {buyer_persona} {industry} challenges frustration",
#         f"site:g2.com {company} reviews",
#         f"site:reddit.com {industry} vendor problems {buyer_persona}",
#     ]
...
#                 all_text.append(f"SOURCE: {r['title']}\n{content[:600]}")

#     if not all_text:
#         logger.info("[NicheResearch] No buyer language found — skipping")
#         return "No buyer language data found."

#     combined = "\n\n---\n\n".join(all_text[:6])
#     logger.info(f"[NicheResearch] ✅ Buyer language fetched ({len(all_text)} sources)")
#     return combined


# # ==================================================
# # Transcript — YouTube captions only, no fallback
# # ==================================================
...
#             return text
#     except Exception:
#         logger.info(f"[NicheResearch] No transcript available for {video_id} — skipping")
#     return None
...
# async def _has_captions(video_id: str, api_key: str) -> bool:
#     url = (
...
#             return any(
#                 i["snippet"].get("trackKind") in ("standard", "asr")
#                 for i in items
#             )
#     except Exception:
#         return False
...
#     unique.sort(key=lambda x: x["engagement"], reverse=True)
#     top_candidates = unique[:12]

#     caption_checks = await asyncio.gather(
#         *[_has_captions(v["video_id"], api_key) for v in top_candidates]
#     )
#     captioned = [v for v, has_cap in zip(top_candidates, caption_checks) if has_cap]
#     logger.info(
#         f"[NicheResearch] {len(captioned)}/{len(top_candidates)} videos "
#         f"have confirmed captions"
#     )

#     top_videos = captioned[:4]
#     logger.info(f"[NicheResearch] Analyzing top {len(top_videos)} captioned videos...")
...
#             logger.info("[NicheResearch] Running all searches in parallel...")
#             (
#                 web_content,
#                 (youtube_content, transcript_count),
#                 buyer_language,
#             ) = await asyncio.gather(
#                 _run_web_searches(web_queries),
#                 _run_youtube_searches(yt_queries),
#                 _fetch_buyer_language(
#                     buyer_persona=entities.get("buyer_persona", ""),
#                     industry=entities.get("industry", metadata.get("business_unit", "")),
#                     company=entities.get("company_name", metadata.get("client", "")),
#                 ),
#             )
...
#                         f"YOUTUBE ANALYSES ({transcript_count} videos):\n{youtube_content}"
#                     )[:12000],
#                     buyer_language=buyer_language[:3000],
#                     client=metadata.get("client", "Unknown"),
#                     project_name=entities.get("project_name", ""),
#                     business_unit=metadata.get("business_unit", "Unknown"),
#                     video_type=metadata.get("video_type", "Unknown"),
...
#             research_brief["niche_summary_title"] = queries.get("niche_summary", "")
#             research_brief["transcript_count"]     = transcript_count
#             research_brief["project_intelligence"] = project_intel
