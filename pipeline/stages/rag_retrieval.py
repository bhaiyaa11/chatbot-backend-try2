import asyncio
import logging
from typing import List, Dict, Optional

from pipeline.stages.base import BaseStage
from google.genai import types
from supabase import create_client, Client
import os
from config import get_genai_client

logger = logging.getLogger(__name__)


class RAGRetrievalStage(BaseStage):
    name = "RAG_RETRIEVAL"

    def __init__(self):
        self.supabase: Client = create_client(
            os.getenv("SUPABASE_URL"),
            os.getenv("SUPABASE_KEY"),
        )
        self._genai_client = None  # lazy init
        self.embedding_model = "text-embedding-004"

    @property
    def genai_client(self):
        if self._genai_client is None:
            self._genai_client = get_genai_client(location="us-central1")
        return self._genai_client

    async def execute(
        self,
        prompt: str,
        metadata: Dict = None,
        match_threshold: float = 0.5,
        match_count: int = 5,
    ) -> List[Dict]:
        """
        Retrieves relevant script chunks from Supabase using vector search.
        Filters by client, business_unit,industries,serviceLines, and video_type if provided in metadata.
        Falls back to unfiltered search if filtered search returns no results.
        """
        logger.info(f"[RAGRetrieval] Embedding prompt: {prompt[:50]}...")

        # ── 1. Generate embedding ────────────────────────────────────────
        try:
            resp = await self.genai_client.aio.models.embed_content(
                model=self.embedding_model,
                contents=prompt,
                config=types.EmbedContentConfig(task_type="RETRIEVAL_QUERY"),
            )
            query_embedding = resp.embeddings[0].values
        except Exception as e:
            logger.error(f"[RAGRetrieval] Embedding failed: {e}")
            return []

        # ── 2. Build metadata filter (drop None/empty values) ────────────
        metadata_filter: Dict = {}
        if metadata:
            for key in ("client", "video_type", "industries", "serviceLines",  "styles", "tone", "duration",):
                val = metadata.get(key)
                if val:
                    metadata_filter[key] = val

        # ── 3. Vector search (with optional metadata fallback) ───────────
        chunks = await self._vector_search(
            query_embedding=query_embedding,
            match_threshold=match_threshold,
            match_count=match_count,
            metadata_filter=metadata_filter,
        )

        # ── 4. Fallback: retry without metadata filter if empty ──────────
        if not chunks and metadata_filter:
            logger.info(
                "[RAGRetrieval] No results with metadata filter — "
                "retrying without filter"
            )
            chunks = await self._vector_search(
                query_embedding=query_embedding,
                match_threshold=match_threshold,
                match_count=match_count,
                metadata_filter={},
            )

        logger.info(f"[RAGRetrieval] Returning {len(chunks)} chunks")
        return chunks

    # ─────────────────────────────────────────────────────────────────────
    # HELPERS
    # ─────────────────────────────────────────────────────────────────────

    async def _vector_search(
        self,
        query_embedding: list,
        match_threshold: float,
        match_count: int,
        metadata_filter: Dict,
    ) -> List[Dict]:
        """
        Runs the Supabase RPC in a thread so it doesn't block the event loop.
        """
        def _sync_call():
            return (
                self.supabase.rpc(
                    "match_chunks",
                    {
                        "query_embedding": query_embedding,
                        "match_threshold": match_threshold,
                        "match_count": match_count,
                        "metadata_filter": metadata_filter,
                    },
                ).execute()
            )

        try:
            rpc_res = await asyncio.get_event_loop().run_in_executor(
                None, _sync_call
            )
            chunks = rpc_res.data or []
            logger.info(
                f"[RAGRetrieval] RPC returned {len(chunks)} chunks "
                f"(filter={metadata_filter or 'none'})"
            )
            return chunks
        except Exception as e:
            logger.error(f"[RAGRetrieval] Vector search failed: {e}")
            return []