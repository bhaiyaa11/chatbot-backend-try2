...
async def call_llm(stage: str, contents: list) -> tuple[dict, int, bool]:
    ...
    NOTE: stage == "CRITIC" is NOT supported here — CRITIC talks to Claude
    directly via the Anthropic API (see generate_text/stream_llm below),
    not Vertex, and this function has no Anthropic branch. Calling
    call_llm("CRITIC", ...) will attempt a Vertex call with an invalid
    model id and fail. Use generate_text("CRITIC", ...) for non-streaming
    JSON critic calls instead.
    """
    # ── Cache check ──────────────────────────────────────────────
...
async def stream_llm(stage: str, contents: list):
    ...
    # ── Anthropic CRITIC ─────────────────────────────────────────
    # Checked FIRST and returns early — CRITIC must never fall through to
    # the Vertex path below, since MODEL_ENDPOINTS["CRITIC"] holds an
    # Anthropic model id ("claude-sonnet-4-6") that Vertex will 404 on.
    if stage == "CRITIC":
        try:
            content_blocks = _build_anthropic_content(contents)
            ...
            logger.error(f"[stream_llm/CRITIC] Anthropic error: {e}")
            raise RuntimeError(f"CRITIC streaming failed: {e}")
    ...
    raise RuntimeError(f"[{stage}] Failed to stream (all endpoints exhausted)")